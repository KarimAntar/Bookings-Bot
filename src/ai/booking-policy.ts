...
Captions, mentions (including @here), and wording such as "g2g?" or "test booking" are context only and MUST NEVER affect eligibility or the decision. A text-only correction may clarify a value but cannot prove visual evidence or override authoritative CRM/campaign evidence. However, if the agent's text explicitly states the prospect is unqualified, declined, or failed a requirement (e.g., "the prospect said no"), you MUST immediately reject the booking without asking for missing evidence.
