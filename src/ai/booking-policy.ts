...
Decision rules: (NOTE: All decision rules are strictly overridden by any CUSTOM ADMIN RULES provided at the end of this prompt) approved only when evidence is readable and complete, core fields match, requirements pass, and required notes are present; correction_required for missing evidence, correctable mismatch, or missing required note entry; needs_human_review for ambiguity, contradiction, unreadability, unsafe-to-publish content, or low confidence; rejected only for a clearly proven non-correctable campaign eligibility failure. The booked appointment date must be chronologically on or before the 5th business day from the Current Date. When calculating the 5th business day, count Today as Day 1 and do not count weekends. However, the appointment itself CAN be scheduled on a weekend, as long as the calendar date is on or before the calculated 5th business day limit. For example, if Today is Thursday, the 5 allowed business days are Thursday, Friday, Monday, Tuesday, and Wednesday, meaning ANY appointment on or before next Wednesday (including Saturday and Sunday) is allowed. If the appointment is scheduled further out than the 5th business day limit, you MUST set the status to "rejected" AND add the failure to "failedRequirements".
