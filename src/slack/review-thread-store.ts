...
interface CreateSession {
  ...
  rootTs: string;
  originalText: string;
  originalImages: readonly ReviewImage[];
  lastEventId: string;
}
...
interface StoredSession extends CreateSession {
...
export interface ActiveReview {
  ...
  lastEventId: string;
  revision: number;
  evidence: { messageText: string; images: readonly ReviewImage[] };
}
...
function validateImages(
  ...
  for (const image of images) {
    if (
      image.source !== source ||
      !image.id.startsWith(`${source}:`) ||
      ids.has(image.id)
    )
      throw new Error(`Invalid or duplicate ${source} image ID`);
    ids.add(image.id);
  }
}
...
export class ReviewThreadStore {
  ...
  create(input: CreateSession): ActiveReview {
    this.prune();
    const key = this.key(input.channel, input.rootTs);
    ...
    if (this.sessions.size >= this.capacity) {
      const oldest = [...this.sessions.entries()].sort(
        ([ka, a], [kb, b]) => a.updatedAt - b.updatedAt || ka.localeCompare(kb),
      )[0];
  ...
  get(channel: string, rootTs: string): ActiveReview | undefined {
    this.prune();
    const stored = this.sessions.get(this.key(channel, rootTs));
    ...
    return {
      ...
      evidence: {
        messageText: [
          stored.originalText,
          correctionText
            ? `Correction (authoritative where conflicting): ${correctionText}`
            : "",
        ]
          .filter(Boolean)
          .join("\n"),
        images: [
          ...stored.originalImages,
          ...(stored.latestCorrection?.images ?? []),
        ],
      },
    };
  }
  has(channel: string, rootTs: string): boolean {
    return this.get(channel, rootTs) !== undefined;
  }
  setOriginalEvidence(
    channel: string,
    rootTs: string,
    ...
    const stored = this.sessions.get(this.key(channel, rootTs));
    if (!stored) return undefined;
    validateImages(images, "original");
    stored.originalText = text;
    stored.originalImages = images;
  ...
  applyCorrection(
    ...
    this.prune();
    const key = this.key(channel, rootTs);
    const stored = this.sessions.get(key);
    if (!stored) return undefined;
    if (revision !== undefined && revision !== stored.revision)
      return undefined;
    ...
    if (!stored.latestCorrection) {
      ...
      stored.latestCorrection = {
        text: stored.latestCorrection.text
          ? `${stored.latestCorrection.text}\n\nCorrection context:\n${correction.text}`
          : correction.text,
    ...
    const now = this.now();
    stored.lastEventId = correction.eventId;
    if (revision === undefined) stored.revision++;
    stored.expiresAt = now + this.ttlMs;
    stored.updatedAt = now;
    return this.get(channel, rootTs);
  }
  close(channel: string, rootTs: string): void {
    this.sessions.delete(this.key(channel, rootTs));
  }
}
