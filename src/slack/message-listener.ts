...
export function registerMessageListener(
  ...
  const queue = new BoundedSemaphore(
    config.maxConcurrentReviews,
    config.maxQueuedReviews,
  );
  ...
  app.event("message", async ({ event, body, client }) => {
    ...
    if (message.channel_type === "im" && !message.bot_id) {
      if (
        message.user &&
        config.adminUserIds.has(message.user) &&
        ruleManager
      ) {
        if (message.text || message.files?.length) {
          try {
            ...
            if (message.files && message.files.length > 0) {
              ...
              downloadedImages = downloaded.map((img) => ({
                mimeType: img.mimeType,
                data: img.data,
              }));
            }
            const response = await ruleManager.handleAdminMessage(
              message.text || "",
              downloadedImages,
            );
            ...
            await client.chat.postMessage({
              channel: message.channel,
              text: "Sorry, I ran into an error processing your rule request.",
            });
          }
        }
      } else {
        await client.chat.postMessage({
          channel: message.channel,
          text: "Sorry, I'm only allowed to learn new rules from authorized admins.",
        });
      }
      return;
    }
    ...
    if (
      config.allowedChannelIds.size &&
      !config.allowedChannelIds.has(message.channel)
    )
      return;
    const rootTs = message.thread_ts ?? message.ts;
    const kind = classifyMessage(
      message,
      Boolean(message.thread_ts && store.has(message.channel, rootTs)),
    );
      ...
      try {
        ...
        await queue.run(async () => {
            ...
            const session =
              kind === "root"
                ...
                : store.applyCorrection(
                    message.channel,
                    rootTs,
                    { text: message.text ?? "", images, eventId },
                    revision,
                  );
          ...
          if (
            ...
            store.close(message.channel, rootTs);
          }
        });
      } catch (error) {
        logger.warn(
    ...
    await current.finally(() => {
      if (threadPasses.get(threadKey) === current)
        threadPasses.delete(threadKey);
    });
  });
}
