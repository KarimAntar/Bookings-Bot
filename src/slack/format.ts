...
export const terminalReaction: Record<ReviewResult["status"], string> = {
  ...
  correction_required: "memo",
  needs_human_review: "warning",
  rejected: "x",
};

const escapeSlack = (value: unknown): string =>
  String(value)
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;");
const bullets = (heading: string, values: readonly string[]) =>
  values.length
...
export function formatReviewResult(result: ReviewResult): string {
  ...
  const heading = {
    approved: "All good! 🎉 Booking approved.",
    correction_required:
      "Hey there! 👋 I need a quick fix before I can approve this.",
    needs_human_review: "Hmm, I need a second pair of eyes on this one.",
    rejected: "I can't approve this one based on the campaign rules.",
  }[result.status];
  const mismatches = result.mismatches.map(
    ({ field, crmValue, bookingValue }) =>
      `${field}: CRM is “${String(crmValue)}”; booking is “${String(bookingValue)}”`,
  );
