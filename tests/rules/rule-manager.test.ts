        ...
        generateContent: mock().mockImplementation(async (req: any) => {
          ...
          if (hasImage && text.includes("Look at this screenshot")) {
            return {
              text: JSON.stringify({
                action: "add",
                payload: "Rule from image",
                response: "Added rule from image.",
              }),
            };
          }
          ...
          return {
            text: JSON.stringify({ action: "unknown", response: "Unknown." }),
          };
        }),
      };
    },
