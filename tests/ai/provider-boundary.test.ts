...
const valid = {
  ...
  notesSummary: {
    present: true,
    contentSummary: "Guest and date present",
...
test("provider sends exactly one ordered package with fixed JSON config", async () => {
  const generateContent = mock(async (_input: any) => ({
  ...
  const { GeminiProvider } = await import(
    `../../src/ai/gemini-provider.ts?capture=${Date.now()}`
  );
  ...
  expect(call.contents[0].parts.filter((x: any) => x.inlineData)).toHaveLength(
    2,
  );
  expect(
    call.contents[0].parts
      .map((x: any) => x.text ?? "")
      .join(" ")
      .indexOf("original:F1"),
  ).toBeLessThan(
