...
const image = (id: string, source: "original" | "correction") => ({
  id,
  name: `${id}.png`,
...
test("labels and orders original then correction evidence in one package", () => {
  ...
  expect(parts[0]).toEqual({
    text: expect.stringContaining("caption/context only"),
  });
  expect(
    parts
      ...
      .join(" "),
  ).toContain("o1 [original]");
  expect(
    parts
      .filter((part) => "text" in part)
      .map((part) => ("text" in part ? part.text : ""))
      .join(" "),
  ).toContain("c1 [correction]");
