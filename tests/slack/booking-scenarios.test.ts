...
function result(overrides: Partial<ReviewResult> = {}): ReviewResult {
  return ReviewResultSchema.parse({
    status: "approved",
    reasoning:
      "CRM, campaign, booking form, qualification answers, and notes match.",
    confidence: 0.98,
    evidenceRoles: [
      { imageId: "original:crm", roles: ["crm_prospect"], readable: true },
      {
        imageId: "original:campaign",
        roles: ["campaign_requirements", "qualification_questions"],
        readable: true,
      },
      {
        imageId: "original:booking",
        roles: ["booking_form", "booking_notes"],
        readable: true,
      },
    ],
    ...
    notesSummary: {
      present: true,
      contentSummary: "Required qualification answers recorded.",
...
class ScenarioProvider implements AIProvider {
  ...
  constructor(
    private readonly decide: (request: ReviewRequest) => ReviewResult,
  ) {}
...
describe("Slack booking scenarios (Listener Flow)", () => {
  ...
  test("approved matching package flows through listener", async () => {
    ...
    try {
        ...
        files: [
          {
            ...
            mimetype: "image/png",
            size: 10,
            url_private_download: "https://fake/crm",
          },
          {
            id: "campaign",
            mimetype: "image/png",
            size: 10,
            url_private_download: "https://fake/campaign",
          },
          {
            id: "booking",
            mimetype: "image/png",
            size: 10,
            url_private_download: "https://fake/booking",
          },
        ],
      };
      ...
      expect(provider.requests[0]?.images.map((i) => i.id)).toEqual([
        "original:crm",
        "original:campaign",
        "original:booking",
      ]);
      ...
      expect(client._reactions.map((r) => r.name)).toContain(
        "white_check_mark",
      );
      ...
      expect(geminiParts[0]).toEqual({
        text: expect.stringContaining(
          "original:crm [original], original:campaign [original], original:booking [original]",
        ),
      });
      expect(geminiParts[1]).toEqual({
        text: expect.stringContaining("Image original:crm [original]"),
      });
    } finally {
  ...
  test("corrected-thread with replacement screenshot flows from root -> correction -> approved", async () => {
    const provider = new ScenarioProvider((request) =>
      request.images.some((img) => img.id.startsWith("correction"))
        ? result()
        : result({
    ...
    try {
      ...
      const correctionMsg: SlackMessage = {
        ...
        ts: "2",
        thread_ts: "1",
        text: "Annual sales: 12",
        files: [
          {
            id: "notes-fixed",
            mimetype: "image/png",
            size: 10,
            url_private_download: "https://fake/notes-fixed",
          },
        ],
      };
      await handler({
      ...
      expect(provider.requests[1]?.images.map((i) => i.id)).toEqual([
        "original:crm",
        "original:campaign",
        "original:booking",
        "correction:notes-fixed",
      ]);
      expect(provider.requests[1]?.messageText).toContain(
        "Correction (authoritative where conflicting): Annual sales: 12",
      );
      ...
      expect(geminiParts[0]).toEqual({
        text: expect.stringContaining(
          "original:crm [original], original:campaign [original], original:booking [original], correction:notes-fixed [correction]",
        ),
      });
    } finally {
      spyOn(global, "fetch").mockRestore();
  ...
  test("minimum-sales failure is rejected", async () => {
    ...
    try {
      ...
      expect(client.posts[0]?.text).toContain(
        "Minimum sales: requires 10, CRM shows 7",
      );
  ...
  test("ambiguous evidence requests human review with <!here>", async () => {
    const provider = new ScenarioProvider(() =>
      result({
        ...
        reasoning: "The sales value is unreadable in the CRM screenshot.",
        confidence: 0.45,
        evidenceRoles: [
          { imageId: "original:crm", roles: ["crm_prospect"], readable: false },
        ],
      }),
    );
    const service = new ReviewService(
  ...
  test("agent text disqualifying the prospect rejects the booking", async () => {
    ...
    try {
      ...
      expect(provider.requests.length).toBe(2);
      expect(client.posts.length).toBe(2);
      expect(client.posts[1]?.text).toContain(
        "The agent explicitly stated the prospect is unqualified.",
      );
      expect(client._reactions.map((r) => r.name)).toContain("x");
