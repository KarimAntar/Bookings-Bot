...
const oversized = {
  ...
  mimetype: "image/png",
  size: 10,
  url_private_download: "https://example.test/private",
};

test("file validation errors do not expose filenames or evidence identifiers", async () => {
  expect(() => selectImageFiles([oversized], 1, 1)).toThrow(
  ...
  try {
    ...
    await expect(
      downloadSlackImage(withoutSize, "token", 1, 1_000),
    ).rejects.toThrow("Image exceeds the size limit");
  } finally {
