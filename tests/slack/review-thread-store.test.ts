...
test("retains original and accumulates corrections", () => {
  ...
  store.create({
    ...
    rootTs: "1",
    originalText: "root",
    originalImages: [image("original:o1")],
    lastEventId: "E1",
  });
  store.applyCorrection("C1", "1", {
    text: "first",
    images: [{ ...image("correction:c1"), source: "correction" }],
    eventId: "E2",
  });
  store.applyCorrection("C1", "1", {
    text: "latest",
    images: [{ ...image("correction:c2"), source: "correction" }],
    eventId: "E3",
  });
  expect(store.get("C1", "1")?.evidence.images.map((item) => item.id)).toEqual([
    "original:o1",
    "correction:c1",
    "correction:c2",
  ]);
...
test("keys by channel and root and evicts oldest at capacity", () => {
  ...
  store.create({
    ...
    rootTs: "same",
    originalText: "",
    originalImages: [image("original:1")],
    lastEventId: "1",
  });
  now++;
  store.create({
    ...
    rootTs: "same",
    originalText: "",
    originalImages: [image("original:2")],
    lastEventId: "2",
  });
