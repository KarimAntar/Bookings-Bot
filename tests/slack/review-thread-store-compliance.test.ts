...
test("validates constructor arguments and refuses duplicate roots", () => {
  ...
  store.create({
    ...
    rootTs: "1",
    originalText: "a",
    originalImages: [image("original:F1")],
    lastEventId: "E1",
  });
  expect(() =>
    store.create({
      ...
      rootTs: "1",
      originalText: "b",
      originalImages: [image("original:F2")],
      lastEventId: "E2",
    }),
  ).toThrow();
...
test("evicts deterministically when timestamps are equal", () => {
  ...
  store.create({
    ...
    rootTs: "b",
    originalText: "",
    originalImages: [image("original:b")],
    lastEventId: "b",
  });
  store.create({
    channel: "C",
    rootTs: "a",
    originalText: "",
    originalImages: [image("original:a")],
    lastEventId: "a",
  });
  store.create({
    channel: "C",
    rootTs: "c",
    originalText: "",
    originalImages: [image("original:c")],
    lastEventId: "c",
  });
...
test("correction images must have stable unique validated IDs", () => {
  ...
  expect(() =>
    store.applyCorrection("C", "1", {
      text: "",
      images: [image("original:F1", "correction")],
      eventId: "E2",
    }),
  ).toThrow();
});
