...
const file = {
  ...
  mimetype: "image/png",
  size: 1,
  url_private_download: "data:image/png;base64,AQ==",
};
const result = (
  status:
    | "approved"
...
function harness(
  review: (request: unknown) => Promise<ReturnType<typeof result>> = async () =>
    result("correction_required"),
) {
  let handler: any;
  const app = {
    event: (_: string, fn: any) => {
      handler = fn;
    },
  } as any;
  const posts: any[] = [];
  const reactions: any[] = [];
  const client = {
    chat: { postMessage: async (x: any) => posts.push(x) },
    reactions: { add: async (x: any) => reactions.push(x) },
  };
...
describe("actual Slack listener", () => {
  test("creates a root session and applies a text-only correction using original evidence", async () => {
    const requests: any[] = [];
    const h = harness(async (request: any) => {
      requests.push(request);
      return result("correction_required");
    });
    await h.handler({
      event: {
        type: "message",
    ...
    expect(requests[1].images.map((x: any) => x.id)).toEqual([
      "original:F1",
      "original:F2",
      "original:F3",
    ]);
...
test("formatter escapes model text and reserves <!here> for human review", () => {
  for (const status of ["approved", "correction_required", "rejected"] as const)
    expect(formatReviewResult(result(status))).not.toContain("<!here>");
