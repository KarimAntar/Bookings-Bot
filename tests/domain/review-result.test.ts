...
const valid = {
  ...
  confidence: 0.95,
  evidenceRoles: [],
  crmFields: { guest: "Ada", nights: 3, prepaid: true, note: null },
  bookingFields: {},
  campaignRequirements: [],
  qualificationQuestions: [],
  notesSummary: {
    present: true,
    contentSummary: "Booking notes verified.",
...
describe("ReviewResultSchema", () => {
  test("parses and normalizes a valid review", () => {
    expect(ReviewResultSchema.parse(valid)).toEqual({
      ...valid,
      reasoning: "Booking details match.",
      flags: ["safe_public_summary"],
    });
  ...
  test("rejects unknown top-level provider fields", () =>
    expect(() =>
      ReviewResultSchema.parse({ ...valid, providerMetadata: "no" }),
    ).toThrow());
  test("rejects confidence outside zero through one", () =>
    expect(() =>
      ReviewResultSchema.parse({ ...valid, confidence: 1.01 }),
    ).toThrow());
  test("rejects empty reasoning and oversized flags", () =>
    expect(() =>
      ReviewResultSchema.parse({
        ...valid,
        reasoning: " ",
        flags: Array.from({ length: 51 }, (_, i) => `f-${i}`),
      }),
    ).toThrow());
});
...
test("humanReviewFallback returns a conservative rich result", () => {
  expect(humanReviewFallback("  ai_timeout  ")).toEqual({
    ...
    missingEvidence: [],
    failedRequirements: [],
    flags: ["ai_timeout", "safe_public_summary"],
  });
});
